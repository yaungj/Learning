...
<font color=red size=7>  fdisk磁盘分区（物理设备之一）--pvcreate创建PV（物理卷-用于注册基础物理设备，LVM自动将PV划分为物理区块PE）--vgcreate创建VG（卷组）--lvcreate创建LV(逻辑卷）--mkfs格式化文件系统--挂载到具体目录--存储使用--扩展LV </font>
